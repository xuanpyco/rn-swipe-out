...
class Swipeout extends React.Component {
   ...
   */
  panResponderObject = {
    onStartShouldSetPanResponder: (info, info2) => true,
    onMoveShouldSetPanResponder: (info, info2) => {
      if (this.props.noSwipeRightThresholdPercentage !== 1 &&
        info2.moveX > this.rightMargin) {
        return false;
      }
  ...
  renderCommonOption = (option, index) => {
    ...
    if (typeof option.component === 'function') {
      return (
        <option.component data={this.props.data} key={index} hide={() => this.performAnimation(0)} animatedValue={this.panAnim} />
      );
    }
    return (
      <Touchable
  ...
  _measureView(event) {
    const { noSwipeRightThresholdPercentage } = this.props;

    if (noSwipeRightThresholdPercentage !== 1) {
      this.rightMargin = (event.nativeEvent.layout.width * noSwipeRightThresholdPercentage);
    };
  }
...
Swipeout.propTypes = {
  ...
  noSwipeRightThresholdPercentage: React.PropTypes.number,
...
Swipeout.defaultProps = {
  ...
  underlayColor: '',
  activeOpacity: 1,
  noSwipeRightThresholdPercentage: 1
};
